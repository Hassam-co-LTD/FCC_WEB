...
export class ApprovedInquiryRecords implements OnInit {
  ...
  loadApprovedTransactions(): void {
    ...
    this.api
      .getAmendRecordTransactionsByStatus(backend)
      .pipe(
        delay(1500),
        finalize(() => {
          this.isLoading = false;
  ...
  async downloadReport(): Promise<void> {
    ...
    if (this.activeTab === 'live') {
      Object.assign(columnStyles, {
        ...
        1: { cellWidth: 22 }, // Created
        2: { cellWidth: 18 }, // Product
        3: { cellWidth: 28 }, // Issuer Reference
        4: { cellWidth: 20 }, // Expiry Date
        5: { cellWidth: 13 }, // Currency
        6: { cellWidth: 22, halign: 'right' }, // Amount
        7: { cellWidth: 30, halign: 'left' }, // Applicant
        8: { cellWidth: 30, halign: 'left' }, // Beneficiary
      });
    } else {
      Object.assign(columnStyles, {
        0: { cellWidth: 25 }, // TNX ID
        1: { cellWidth: 18 }, // Event
        2: { cellWidth: 38 }, // Event Ref No
        3: { cellWidth: 12 }, // Event Seq.
        4: { cellWidth: 23 }, // Created
        5: { cellWidth: 20 }, // Product
        6: { cellWidth: 18 }, // Issuer Reference

...
ngOnInit() {
  ...
  get instructions(): FormGroup { return this.undertakingForm.get('instructions') as FormGroup; }
    ...
    saveForm(): void {
        ...
        this.api.saveUtgAmendTransaction(tnxId, payload).pipe(
          ...
          next: (res: UndertakingGuarantee) => {
            ...
            this.snackBar.open(
              ...
              'Close',
              { duration: 5000 });
            setTimeout(() => this.router.navigate(['/dashboard/Trade-Services/undertaking-issuance/approved-inquiry-records']),50 
            );
          },
          error: () => {
  ...
  approve(): void {
    ...
    this.api.approveUtgAmendment(eventRefNo, payload).subscribe({
      next: () => {
        ...
        setTimeout(() => this.router.navigate(['/dashboard/Trade-Services/undertaking-issuance/inquiries-records']), 50
        );
      },
...
openReject(): void {
    ...
    this.router.navigate(['/dashboard/Trade-Services/undertaking-issuance/approved-inquiry-records'], {
      queryParams: { tab }
    });
  }
...
updateRejected(): void {
    ...
    this.api.updateRejectedUndertaking(payload.tnxId, payload).subscribe({
      next: (res) => {
        ...
        this.router.navigate(['/dashboard/Trade-Services/undertaking-issuance/inquiries'],);
      },
      error: () => {

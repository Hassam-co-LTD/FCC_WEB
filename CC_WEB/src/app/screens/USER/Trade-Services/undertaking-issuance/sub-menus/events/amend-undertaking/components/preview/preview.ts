...
export class Preview implements OnInit {
  ...
  submitForm(): void {
    ...
    this.api.submitUtgAmendment(tnxId, this.currentTx!).subscribe({
      next: (res) => {
        this.router.navigate(
  ...
  approveTransaction(): void {
    ...
    this.api
      .approveUtgAmendment(this.currentTx.tnxId, this.currentTx)
      .subscribe({
        next: (res) => {
  ...
  rejectTransaction(): void {
    ...
    dialogRef.afterClosed().subscribe((reason: string | undefined) => {
      ...
      this.api.rejectUtgAmendment(tnxId, reason).subscribe({
        next: (res) => {

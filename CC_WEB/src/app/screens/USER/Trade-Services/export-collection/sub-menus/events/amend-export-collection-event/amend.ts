...
attachments: this.fb.array([])
  ...
  private enterEditMode(tnxId: string): void {
    ...
    if (this.eventRefNo) {
      ...
      this.api.getAmendmentByEventRefNoExportCollection(this.eventRefNo).subscribe({
        next: (event) => {
        ...
        error: () => {
          ...
          this.api.getTransactionByTnxIdExportCollection(tnxId).subscribe({
            next: (tx) => {
              ...
              this.currentTx = { tnxId: tx.tnxId } as ExportCollectionTransaction;
              this.patchForm(tx);
              this.screenMode = 'EDIT';
              this.ExportCollectionForm.enable();
            },
  ...
  get generalDetails(): FormGroup { return this.ExportCollectionForm.get('generalDetails') as FormGroup; }
  get DrawerDraweeDetails(): FormGroup { return this.ExportCollectionForm.get('DrawerDraweeDetails') as FormGroup; }
  get bankDetails(): FormGroup { return this.ExportCollectionForm.get('bankDetails') as FormGroup; }
  get PaymentAndAmount(): FormGroup { return this.ExportCollectionForm.get('PaymentAndAmount') as FormGroup; }
  get ShipmentDetails(): FormGroup { return this.ExportCollectionForm.get('ShipmentDetails') as FormGroup; }
  get CollectionInstruction(): FormGroup { return this.ExportCollectionForm.get('CollectionInstruction') as FormGroup; }
  get attachmentsArray(): FormArray { return this.ExportCollectionForm.get('attachments') as FormArray; }
  ...
  saveForm(): void {
    ...
    this.api.saveamendTransactionExportCollection(tnxId, payload).pipe(
    ...
    this.api.submitAmendmentExportCollection(eventRefNo,payload).subscribe({
      next: (res) => {
        ...
        this.snackBar.open(
          ...
          'Close',
          { duration: 5000 });
        setTimeout(() => this.router.navigate(['/dashboard/Trade-Services/export-collection/approved-inquiry-records']), 50
        );
      },
  ...
  approve(): void {
    ...
    this.api.approveAmendmentExportCollection(eventRefNo, payload).subscribe({
      next: () => {
        ...
        setTimeout(() => this.router.navigate(['/dashboard/Trade-Services/export-collection/inquiries-records']), 50
        );
      },
  ...
  private navigateBack(tab: string) {
    this.router.navigate(['/dashboard/Trade-Services/export-collection/approved-inquiry-records'], {
      queryParams: { tab }
    });
  }
  ...
  updateRejected(): void {
    ...
    this.api.updateRejectedTransactionExportCollection(payload.tnxId, payload).subscribe({
      next: (res) => {
        this.snackBar.open(

...
export class ApprovedInquiryRecords implements OnInit {
  ...
  async downloadReport(): Promise<void> {
    ...
    const hasFilters =
      !!this.searchQuery?.trim() || !!this.currencyFilter?.trim();
    ...
    autoTable(doc, {
      ...
      didDrawPage: () => {
        ...
        doc.text('Export Collection Records', 10, pageHeight - 7);
    ...
    // File Name
    // =========================
    const fileName = `Export_Collection_${this.activeTab}_Report_${this.getCurrentDate()}.pdf`;
  ...
  // EXACTLY MATCHES HTML TABLE
  // ============================================================
  private getReportRow(tx: ExportCollectionTransaction): any[] {
    // =========================
    // LIVE
    // =========================
    if (this.activeTab === 'live') {
  ...
  // EXACTLY MATCHES HTML TABLE
  // ============================================================
  private getExcelRow(tx: ExportCollectionTransaction): any[] {
    // =========================
    // LIVE
    // =========================
    if (this.activeTab === 'live') {
  ...
  private downloadExcel(): void {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Export Collection');

    // =========================
    // File Name
    // =========================
    const fileName = `Export_Collection_${this.activeTab}_Report_${this.getCurrentDate()}.xlsx`;

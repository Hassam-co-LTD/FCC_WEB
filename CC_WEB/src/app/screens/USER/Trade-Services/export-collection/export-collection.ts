...
attachments: this.fb.array([])
  ...
  saveForm(): void {
    ...
    this.api.savePendingExportCollection(payload).subscribe({
      next: (res: ExportCollectionTransaction) => {
        ...
        setTimeout(() => this.router.navigate(['dashboard/Trade-Services/export-collection/inquiries-records'], { queryParams: { tab: 'pending' } }
        ), 50);
      },
  ...
  update(): void {
    ...
    this.api.updatePendingByTnxIdExportCollection(payload).subscribe({
      next: (res) => {
        ...
        setTimeout(
          () => this.router.navigate(['dashboard/Trade-Services/export-collection/inquiries-records'], { queryParams: { tab: 'pending' } }),
          300
        );
      },
      error: () => {
  ...
  openReject(): void {
    ...
    dialogRef.afterClosed().subscribe((reason: string | undefined) => {
      ...
      this.api.rejectTransactionExportCollection(this.currentTx.tnxId!, reason).subscribe({
        next: (res) => {
